...
list_ticker <-read.csv('./cockpit/tickers_with_param_maxit.csv')

df_result <- NULL
for (j in 1:nrow(list_ticker)) {
  
  entry <- list_ticker[j,]
  
  fileName <- paste(entry$ticker, ".csv", sep='')
  ...
  date_txt_from <- paste(first_row_timeserie$Date)
  date_txt_to_base <- paste(last_row_timeserie$Date)
  
  
  nbre_step_backward <- entry$step_backward
  nbre_generation <- entry$maxit
  ...
  for (i in 0:nbre_step_backward) {
    ...
    if (as.POSIXlt(to)$wday != 0 & as.POSIXlt(to)$wday != 6) { #saute weekend
      ...
      rbind(df_result, c(fileSName, date_txt_from, format(to, "%Y-%m-%d"), last_row$t, last_row$Close, -i, nbre_generation, test$par[3]-last_row$t, as.integer((test$par[3]-last_row$t)/(1/365)), test$par[1], test$par[2], test$par[3], linear_param[1], linear_param[2], linear_param[3], linear_param[4])) -> df_result
...
write.csv(df_result, paste('./cockpit/', 'mini_cockpit_maxit_', nowdatetime, "_from_", date_txt_from, "_to_", date_txt_to_base, ".csv", sep=''), row.names = FALSE)